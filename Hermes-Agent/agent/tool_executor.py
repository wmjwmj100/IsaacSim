...
def _tool_result_preview_and_length(result: Any, *, verbose: bool, limit: int = 200) -> tuple[str, int]:
    """Return a string preview and text length for plain or multimodal results."""
    text = _multimodal_text_summary(result)
    preview = text if verbose else (text[:limit] if len(text) > limit else text)
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    def _run_tool(index, tool_call, function_name, function_args, middleware_trace):
        ...
        try:
            ...
            result_preview, result_len = _tool_result_preview_and_length(
                result,
                verbose=False,
            )
            if is_error:
                logger.info("tool %s failed (%.2fs): %s", function_name, duration, result_preview)
            else:
                logger.info("tool %s completed (%.2fs, %d chars)", function_name, duration, result_len)
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        result_preview, _result_len = _tool_result_preview_and_length(
            function_result,
            verbose=agent.verbose_logging,
        )
