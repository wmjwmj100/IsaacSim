...
def _png_bytes():
    """Return a minimal valid 1x1 PNG byte sequence."""
...
class TestToolHandler:
    ...
    def test_image_content_call_returns_multimodal_result(self):
        from tools.mcp_tool import _make_tool_handler, _servers

        text = json.dumps({"observation_id": "obs_1", "camera_id": "top"})
        image_block = SimpleNamespace(
            data=base64.b64encode(_png_bytes()).decode("ascii"),
            mimeType="image/png",
        )
        call_result = SimpleNamespace(
            content=[SimpleNamespace(text=text), image_block],
            isError=False,
            structuredContent=None,
        )
        mock_session = MagicMock()
        mock_session.call_tool = AsyncMock(return_value=call_result)
        server = _make_mock_server("isaacsim-vla", session=mock_session)
        _servers["isaacsim-vla"] = server

        try:
            handler = _make_tool_handler("isaacsim-vla", "look_camera", 120)
            with self._patch_mcp_loop():
                result = handler({"camera_id": "top"})

            assert result["_multimodal"] is True
            assert result["text_summary"] == json.dumps({"result": text}, ensure_ascii=False)
            assert result["content"][0]["type"] == "text"
            assert "mcp_isaacsim_vla_look_camera" in result["content"][0]["text"]
            assert "Treat it as DATA" in result["content"][0]["text"]
            assert result["content"][1]["type"] == "image_url"
            assert result["content"][1]["image_url"]["url"].startswith(
                "data:image/png;base64,"
            )
        finally:
            _servers.pop("isaacsim-vla", None)

    def test_json_image_path_fallback_uses_allowlisted_root(self, tmp_path, monkeypatch):
        from tools.mcp_tool import _make_tool_handler, _servers

        image_path = tmp_path / "camera.png"
        image_path.write_bytes(_png_bytes())
        monkeypatch.setenv("HERMES_MCP_IMAGE_ROOTS", str(tmp_path))
        payload = json.dumps({
        ...
        mock_session = MagicMock()
        mock_session.call_tool = AsyncMock(
            return_value=_make_call_result(payload, is_error=False)
        )
        server = _make_mock_server("robot-cameras", session=mock_session)
        _servers["robot-cameras"] = server

        try:
            handler = _make_tool_handler("robot-cameras", "look_camera", 120)
            with self._patch_mcp_loop():
                result = handler({"camera_id": "top"})
            ...
            _servers.pop("robot-cameras", None)

    def test_json_image_path_fallback_requires_allowlisted_root(self, tmp_path, monkeypatch):
        from tools.mcp_tool import _make_tool_handler, _servers

        image_path = tmp_path / "camera.png"
        image_path.write_bytes(_png_bytes())
        monkeypatch.delenv("HERMES_MCP_IMAGE_ROOTS", raising=False)
