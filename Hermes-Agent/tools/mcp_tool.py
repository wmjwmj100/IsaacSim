...
def _mcp_image_block_to_image_url_part(block) -> Optional[dict]:
    """Convert an MCP ``ImageContent`` block to an OpenAI-style image_url part."""
    data = getattr(block, "data", None)
    mime_type = getattr(block, "mimeType", None)
    normalized_mime = str(mime_type or "").split(";", 1)[0].strip().lower()
    if data is None or not normalized_mime.startswith("image/"):
    ...
    try:
        ...
        if not _looks_like_image(raw_bytes):
            logger.warning("MCP image block rejected: bytes do not look like an image")
            return None
    except ImportError:
        pass
    ...
    return {
        "type": "image_url",
        "image_url": {"url": f"data:{normalized_mime};base64,{data}"},
    }


def _mcp_multimodal_text_block(tool_name: str, text: str) -> dict:
    """Build the text side of a multimodal MCP result.

    ``make_tool_result_message`` only wraps plain string MCP outputs. Multipart
    content must keep list shape for vision providers, so add the same data
    framing directly to the text part.
    """
    return {
        "type": "text",
        "text": (
...
def _mcp_local_image_roots(server_name: str) -> list[Path]:
    """Return configured local image roots trusted for MCP JSON path fallback."""
    raw = os.getenv("HERMES_MCP_IMAGE_ROOTS", "").strip()
    roots: list[Path] = []
    ...
    if safe_server == "isaacsim_vla":
        for env_name in ("ISAACSIM_VLA_WORKDIR", "ISAACSIM_REPO_ROOT"):
            value = os.getenv(env_name, "").strip()
            if value:
                roots.append(Path(value).expanduser())
        roots.append(Path("/work/IsaacSim/outputs"))
    return roots


def _guess_image_mime_from_path(path: Path) -> Optional[str]:
    import mimetypes
...
def _local_image_path_to_part(path_value: str, *, server_name: str) -> Optional[dict]:
    """Safely convert an allowlisted local image path to an image_url part."""
    ...
    if not path.is_file():
        return None
    roots = _mcp_local_image_roots(server_name)
    if not roots:
        return None
    if not any(_path_is_under(path, root) for root in roots):
        logger.debug(
            "MCP server '%s': refusing to inline image outside allowed roots: %s",
            server_name, path,
        )
        return None
    mime_type = _guess_image_mime_from_path(path)
    if not mime_type:
        return None
    ...
    try:
        ...
        logger.debug("MCP local image read failed for %s: %s", path, exc)
        return None
    try:
        from gateway.platforms.base import _looks_like_image
...
def _extract_mcp_json_image_parts(text_result: str, *, server_name: str) -> list[dict]:
    """Extract allowlisted local image paths from JSON MCP text fallback."""
    ...
    def _walk(value: Any) -> None:
        if isinstance(value, dict):
            for key, nested in value.items():
                if key in {"image_path", "overlay_path", "original_image_path"} and isinstance(nested, str):
    ...
    for candidate in candidates:
        if candidate in seen:
            continue
        seen.add(candidate)
        part = _local_image_path_to_part(candidate, server_name=server_name)
        if part:
            parts.append(part)
    return parts


def _make_mcp_tool_result_payload(
    *,
    server_name: str,
    tool_name: str,
    text_result: str,
    ...
    """Return a string JSON result or Hermes multimodal tool-result envelope."""
    if image_parts and not text_result and structured is None:
        text_result = "[MCP image content attached]"
    if structured is not None:
        if text_result:
            ...
            base_payload = {"result": structured}
    else:
        base_payload = {"result": text_result}
    text_payload = json.dumps(base_payload, ensure_ascii=False)
    ...
    return {
        "_multimodal": True,
        "content": [
            _mcp_multimodal_text_block(prefixed_tool_name, text_payload),
            *image_parts,
        ],
        "text_summary": text_payload,
    }
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # Collect text and image blocks. Text stays as JSON metadata; MCP
            # ImageContent blocks become Hermes' existing multimodal tool-result
            # envelope so a vision-capable active model can inspect screenshots
            # and robotics camera frames directly.
            parts: List[str] = []
            image_parts: List[dict] = []
            for block in (result.content or []):
                if hasattr(block, "text") and block.text:
                    ...
                    continue
                image_part = _mcp_image_block_to_image_url_part(block)
                if image_part:
                    image_parts.append(image_part)
            text_result = "\n".join(parts) if parts else ""
            if not image_parts:
                image_parts = _extract_mcp_json_image_parts(
                    text_result,
            ...
            return _make_mcp_tool_result_payload(
                server_name=server_name,
                tool_name=tool_name,
