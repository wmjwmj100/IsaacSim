...
def _first_action_step(value: Any) -> list[float]:
    ...
    return [float(item) for item in value[:7]]
...
def _as_vector7(value: Any, key: str) -> list[float]:
    value = _to_serializable(value)
    array = np.asarray(value, dtype=np.float64).reshape(-1)
    if array.shape[0] != 7:
        raise RuntimeError(f"{key} must be 7D, got shape {array.shape}")
...
def main() -> None:
    ...
    dataset_kwargs: dict[str, Any] = {"root": Path(args.dataset_root).expanduser()}
    ...
    for offset in offsets:
        ...
        gt_action = _as_vector7(frame["action"], "action")
        obs_state = _as_vector7(frame["observation.state"], "observation.state")
