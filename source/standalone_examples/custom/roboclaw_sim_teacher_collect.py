...
GRIPPER_LOWER = 0.02897806651890278
GRIPPER_UPPER = 0.060192931443452835
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--trajectory-mode",
        choices=("ik_push", "scripted_motion"),
        default=os.getenv("ROBOCLAW_TEACHER_TRAJECTORY_MODE", "ik_push"),
        help="ik_push keeps the original cube-push teacher. scripted_motion records a fixed free-space arm motion.",
    )
    ...
    parser.add_argument("--scripted-forward-m", type=float, default=0.12)
    parser.add_argument("--scripted-left-m", type=float, default=0.10)
    parser.add_argument("--scripted-lift-m", type=float, default=0.025)
    parser.add_argument("--scripted-gripper", type=float, default=0.055)
...
def action_target_for_dofs(dof_names: list[str], action7: list[float]) -> np.ndarray:
    gripper = float(np.clip(action7[6], GRIPPER_LOWER, GRIPPER_UPPER))
    targets = {
...
def solve_position_ik(
    ...
    action7 = [float(value) for value in target6.tolist()] + [float(np.clip(gripper, GRIPPER_LOWER, GRIPPER_UPPER))]
    diagnostics = {
...
def scripted_motion_goal(
    ...
    lift = np.array([0.0, 0.0, float(args.scripted_lift_m)], dtype=np.float64)
    forward = np.array([float(args.scripted_forward_m), 0.0, 0.0], dtype=np.float64)
    left = np.array([0.0, float(args.scripted_left_m), 0.0], dtype=np.float64)

    start_hover = start + lift
    forward_end = start_hover + forward
    left_end = forward_end + left

    if progress < 0.20:
        phase = "settle"
        goal = lerp(start, start_hover, smoothstep(progress / 0.20))
    elif progress < 0.48:
        phase = "forward"
        goal = lerp(start_hover, forward_end, smoothstep((progress - 0.20) / 0.28))
    elif progress < 0.78:
        phase = "left"
        goal = lerp(forward_end, left_end, smoothstep((progress - 0.48) / 0.30))
    else:
        phase = "return"
        goal = lerp(left_end, start_hover, smoothstep((progress - 0.78) / 0.22))

    gripper = float(args.scripted_gripper) + 0.002 * math.sin(2.0 * math.pi * progress)
    return goal, phase, gripper
...
def run_collection() -> dict[str, Any]:
    ...
    cameras = {
        ...
        "camera2": build_camera(wrist_name, wrist_camera_path),
    }
    ...
    for episode_index in range(int(args.episodes)):
        if args.trajectory_mode == "scripted_motion":
            cube_position = np.asarray(initial_cube_position, dtype=np.float64).copy()
            push_direction = np.array([0.0, 1.0], dtype=np.float64)
        else:
        ...
        scripted_start_finger = finger_midpoint()
        ...
        for frame_index in range(int(args.frames_per_episode)):
            ...
            if args.trajectory_mode == "scripted_motion":
                goal, phase, gripper = scripted_motion_goal(
                    frame_index,
                    int(args.frames_per_episode),
                    scripted_start_finger,
                )
            else:
                goal, phase, gripper = teacher_goal(
                    frame_index,
                    int(args.frames_per_episode),
                    episode_start_cube,
                    push_direction,
            ...
            camera2_path = frame_dir / f"frame_{frame_index:06d}_camera2_{wrist_name}.png"
            if should_save:
                write_png_rgb(camera1_path, camera_rgb("camera1", cameras["camera1"]))
                write_png_rgb(camera2_path, camera_rgb("camera2", cameras["camera2"]))
            ...
            record = {
                ...
                "camera2_source": wrist_camera_path,
        ...
        final_finger = finger_midpoint()
        scripted_finger_path_xy_m = xy_displacement(scripted_start_finger, final_finger)
        if args.trajectory_mode == "scripted_motion":
            success = True
        else:
        ...
        episode_summary = {
            ...
            "scripted_start_finger_position": scripted_start_finger,
            "scripted_final_finger_position": final_finger,
            "scripted_finger_path_xy_m": scripted_finger_path_xy_m,
    ...
    summary = {
        ...
        "camera_paths": {"camera1": args.overhead_camera_path, "camera2": wrist_camera_path},
        "task": args.task_text,
        "trajectory_mode": args.trajectory_mode,
        "fps": int(args.fps),
